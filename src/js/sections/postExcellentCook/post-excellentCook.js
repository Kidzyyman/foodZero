class ColorManager {
  ...
  changeColor() {
    const previousIndex = (this.currentColorIndex - 1 + this.images.length) % this.images.length
    this.images[previousIndex].style.fill = 'black'
    this.images[this.currentColorIndex].style.fill = this.getRandomColorByIndex(this.currentColorIndex)
  ...
  startColorChange() {
    ...
    if (scrollHeight > 4450 && scrollHeight < 5500) {
      if (!this.colorChangeInterval) {
        this.changeColor()
        this.colorChangeInterval = setInterval(this.changeColor.bind(this), 3000)
      }
    } else {
...
export class PostExcellentCook {
  constructor() {
    const images = document.querySelectorAll('.post-excellentCook-item__icon path')
